...
export const heroImageTool = new FunctionTool({
  ...
  execute: async (args: any) => {
    const { prompt, slug, outputDir, runId } = args;
    ...
    return new Promise((resolve) => {
      execFile(nodeExec, [script, 'hero', prompt, '--slug', slug, '--output', safeOutputDir], {
        cwd: seoRoot,
        env: toolEnv,
        timeout: 180_000,
      }, (error, stdout, stderr) => {
...
export const ogImageTool = new FunctionTool({
  ...
  execute: async (args: any) => {
    const { slug, title, heroPath, outputDir, runId } = args;
    const seoRoot = resolveSeoRoot(['scripts/image-pipeline.mjs']);
    const safeOutputDir = resolveManagedOutputDir(seoRoot, outputDir, runId);
    const nodeExec = resolveNodeExec();
    const toolEnv = buildToolEnv(seoRoot);
    ...
    // If caller passes an old/non-mounted hero path, fall back to this run's managed output.
    const preferredHeroPath = fs.existsSync(heroPath)
      ? heroPath
      : path.join(safeOutputDir, 'assets', 'hero-4k.png');

    return new Promise((resolve) => {
      execFile(nodeExec, [script, 'og', '--slug', slug, '--title', title, '--hero', preferredHeroPath, '--output', safeOutputDir], {
        cwd: seoRoot,
        env: toolEnv,
        timeout: 60_000,
      }, (error, stdout, stderr) => {
